...
const passwordSchema = Joi.string().min(8).max(128).pattern(/[A-Z]/, 'uppercase letter').pattern(/[0-9]/, 'number').messages({
...
export const Schemas = {
  ...
  rewardRedemption: {
    ...
    updateStatus: Joi.object({
      status: Joi.string().valid('pending', 'approved', 'redeemed', 'cancelled', 'expired').required(),
  ...
  restaurant: {
    create: Joi.object<IRestaurant>({
      profile: Joi.object({
        ...
        category: Joi.array()
          .items(Joi.string().valid(...categoryEnum))
          .min(1)
          .required(),
        globalRating: Joi.number().min(0).max(10),
    ...
    update: Joi.object({
      ...
      profile: Joi.object({
        ...
        maxPointsVisit: Joi.number().min(0).allow(null),
        category: Joi.alternatives().try(Joi.array().items(Joi.string()), Joi.string().allow('', null)),
        timetable: Joi.any(),
        image: Joi.array().items(Joi.string()),
        contact: Joi.object({
  ...
  dish: {
    create: Joi.object<IDish>({
      ...
      availableAt: Joi.array()
        .items(Joi.string().valid('breakfast', 'brunch', 'lunch', 'happy-hour', 'dinner', 'all-day'))
        .required(),
