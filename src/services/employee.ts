...
const getAllDeletedEmployees = async (skip: number, limit: number): Promise<{ employees: IEmployee[]; total: number }> => {
    const [employees, total] = await Promise.all([
        EmployeeModel.find({ isActive: false })
            .select('-profile.password')
            .skip(skip)
            .limit(limit)
            .lean<IEmployee[]>(),
...
const getByRestaurantWithStats = async (restaurant_id: string): Promise<EmployeeStatsResult[]> => {
    ...
    const employeeIds = employees
        ...
        .filter((employeeId): employeeId is mongoose.Types.ObjectId => Boolean(employeeId));
    ...
    const reviewsMap = new Map<string, { averageRating: number | null }>(
        reviewsStats.map((item) => [String(item._id), { averageRating: item.averageRating }])
    );
