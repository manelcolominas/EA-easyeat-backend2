...
export async function insertRewardVector(
  data: IRewardWeaviate
): Promise<void> {
...
export async function searchRestaurants(
  ...
  return Promise.all(
    result.objects.map((obj) => resolveIdsToNames(obj.properties))
  );
}
 ...
 */
export async function getRestaurantsByMongoIds(
  mongoIds: string[]
): Promise<Record<string, any>[]> {
...
export async function searchDishes(
  queryEmbedding: number[],
  limit: number = 10
): Promise<{ resolved: Record<string, any>[]; restaurantIds: string[] }> {
  const client = await getWeaviateClient();
  const collection = client.collections.use("Dish");
  ...
  const restaurantIds = result.objects
    .map((obj) => obj.properties.restaurantId as string)
