...
const getCustomerAllPointsWallet = async (customer_id: string, skip: number, limit: number): Promise<{ pointsWallet: IPointsWallet[]; total: number }> => {
  ...
  const [pointsWallet, total] = await Promise.all([
    PointsWalletModel.find(filter).populate('restaurant_id', 'profile.name profile.location').skip(skip).limit(limit).lean<IPointsWallet[]>(),
...
const getCustomerAllVisits = async (customer_id: string, skip: number, limit: number): Promise<{ visits: IVisit[]; total: number }> => {
  ...
  const [visits, total] = await Promise.all([
    VisitModel.find(filter).populate('restaurant_id', 'profile.name profile.rating profile.location.city').sort({ createdAt: -1 }).skip(skip).limit(limit).lean<IVisit[]>(),
...
const getCustomerAllFavouriteRestaurants = async (customer_id: string, skip: number, limit: number): Promise<{ favoriteRestaurants: IRestaurant[]; total: number }> => {
  ...
  const [restaurants, total] = await Promise.all([
    RestaurantModel.find(filter).select('profile.name profile.description profile.globalRating profile.category profile.image profile.location.city').skip(skip).limit(limit).lean<IRestaurant[]>(),
...
const getCustomerAllBadges = async (customer_id: string, skip: number, limit: number): Promise<{ badges: IBadge[]; total: number }> => {
  ...
  const [badges, total] = await Promise.all([BadgeModel.find(filter).select('title description type').skip(skip).limit(limit).lean<IBadge[]>(), BadgeModel.countDocuments(filter)]);
...
const getCustomerAllReviews = async (customer_id: string, skip: number, limit: number): Promise<{ reviews: IReview[]; total: number }> => {
  ...
  const [reviews, total] = await Promise.all([
    ReviewModel.find(filter).populate('restaurant_id', 'profile.name profile.rating').sort({ createdAt: -1 }).skip(skip).limit(limit).lean<IReview[]>(),
...
const getAllCustomers = async (skip: number, limit: number): Promise<{ data: ICustomer[]; total: number }> => {
  ...
  const [data, total] = await Promise.all([CustomerModel.find(filter).skip(skip).limit(limit).lean<ICustomer[]>(), CustomerModel.countDocuments(filter)]);
  return { data, total };
};
