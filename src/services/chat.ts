...
export class ChatService {
    ...
    public async createMessage({
        ...
        return Chat.findById(message._id)
            .populate('customer')
            .populate('restaurant');
    }
    ...
    public inicializarSockets(): void {
        ...
        this.io.on('connection', (socket: Socket) => {
            ...
            socket.on(
                'chat:joinCustomer',
                ({ customerId }: JoinCustomerPayload) => {
                    if (!customerId) {
                        socket.emit('chat:error', {
            ...
            socket.on(
                'chat:joinRestaurant',
                ({ restaurantId }: JoinRestaurantPayload) => {
                    if (!restaurantId) {
                        socket.emit('chat:error', {
            ...
            socket.on('chat:sendMessage', async (payload: SendMessagePayload) => {
                try {
                    const { conversationId, senderId, senderRole, contenido } =
                        payload;

                    if (!conversationId || !senderId || !senderRole || !contenido) {
                        socket.emit('chat:error', {
                            message:
                                'conversationId, senderId, senderRole and contenido are required',
                        });
                        return;
                    }
                    ...
                    const restaurantId = this.getEntityId(
                        (message as any).restaurant
                    );

                    const customerId = this.getEntityId(
                        (message as any).customer
                    );
                    ...
                    this.io!.to(`restaurant:${restaurantId}`).emit(
                    ...
                    this.io!.to(`customer:${customerId}`).emit(
                        'chat:conversationUpdated',
                        {
                            conversationId,
                            message,
