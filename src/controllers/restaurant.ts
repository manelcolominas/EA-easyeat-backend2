...
const getRestaurantsNearby = async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
  ...
  try {
    const restaurants = await RestaurantService.getReestaurantsNearby(parseFloat(lng as string), parseFloat(lat as string), maxDistance ? parseFloat(maxDistance as string) : 5_000);
